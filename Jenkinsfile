        ...
        parallel(
          ...
          "Cypress": {
            node(label: 'docker') {
              script {
                try {
                  sh '''docker pull eeacms/eea-website-backend; docker run --rm -d --name="$BUILD_TAG-plone" -e SITE="Plone" -e ADDONS="$ADDONS" -e PROFILES="$PROFILES" eeacms/eea-website-backend'''
